...
SplashTextOn, , 60,Radmir CRMP server 04, Запуск скрипта. Ожидайте..`nПроверяем наличие обновлений.
URLDownloadToFile, %downllen%, %a_temp%/varleni.ini
IniRead, buildupd, %a_temp%/varleni.ini, UPD, build
if buildupd =
{
    SplashTextOn, , 60,Radmir CRMP server 04, Запуск скрипта. Ожидайте..`nОшибка. Нет связи с сервером.
    sleep, 2000
}
if buildupd > % buildscr
{
    IniRead, vupd, %a_temp%/varleni.ini, UPD, v
    SplashTextOn, , 60,Radmir CRMP server 04, Запуск скрипта. Ожидайте..`nОбнаружено обновление до версии %vupd%!
    sleep, 2000
    IniRead, desupd, %a_temp%/varleni.ini, UPD, des
    desupd := Utf8ToAnsi(desupd)
    IniRead, updupd, %a_temp%/varleni.ini, UPD, upd
    ...
    IfMsgBox OK
    {
        ...
        IfMsgBox OK
        {
            ...
            SplashTextOn, , 60,Radmir CRMP server 04, Обновление. Ожидайте..`nОбновляем скрипт до версии %vupd%!
...
{FFD933}=====================================================`n`n
	...
	ShowDialog(0, "{ffffff}Admin | {FFD933}stats", "{FFFFFF}Ваш никнейм: `t`t`t`t`t{05A9FF}" nick "{FFFFFF}`n`n" statsnak "{FFFFFF}Текущее время:`t`t`t`t{05A9FF}" time " - h:m:s{FFFFFF}`nТекущая дата:`t`t`t`t`t{05A9FF}" date "", "Закрыть")
}
return
